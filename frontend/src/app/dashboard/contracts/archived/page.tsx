...
import { Search, FileText, Eye, Archive, RefreshCcw } from "lucide-react";
...
const ArchivedContracts = () => {
  ...
  const stats = useMemo(() => {
    if (!Array.isArray(contracts)) return { total: 0, totalValue: 0, recentlyArchived: 0 };
    ...
    return {
      ...
      totalValue,
      recentlyArchived: contracts.filter(c => c.updated_at && new Date(c.updated_at) > thirtyDaysAgo).length
    };
  }, [contracts]);
    ...
    return (
      <div className="space-y-8 p-6 animate-fade-in min-h-screen" style={{ backgroundColor: '#f0eff4' }}>
        <div className="flex justify-between items-center">
          <h2 className="text-3xl font-bold text-gray-900">Archived Contracts</h2>
        </div>
        <SkeletonStats />
        <SkeletonTable rows={8} />
        <div className="flex items-center justify-center py-8">
          <LoadingSpinner size="lg" showText text="Loading archived contracts..." />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-6" style={{ backgroundColor: '#f0eff4' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="bg-white">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-muted-foreground">Recently Archived</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-foreground">{stats.recentlyArchived}</div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gradient-to-r from-muted/50 to-muted/30 border-b border-border/50">
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Archived On</th>
              ...
              {filteredContracts.length > 0 ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    ...
                    <div className="text-muted-foreground">{searchQuery ? "No contracts found matching your criteria" : "No archived contracts"}</div>
                  </td>
                </tr>
              )}
            </tbody>
