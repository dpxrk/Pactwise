  ...
  useEffect(() => {
    ...
    const fetchTask = async () => {
      ...
      setTask({
        ...
        startedAt: data.started_at ? new Date(data.started_at) : (data.created_at ? new Date(data.created_at) : new Date()),
    ...
    const fetchTasks = async () => {
      ...
      for (const task of data) {
        ...
        taskMap.set(task.id, {
          ...
          status,
          startedAt: task.started_at ? new Date(task.started_at) : (task.created_at ? new Date(task.created_at) : new Date()),
