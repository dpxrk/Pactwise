...
// Updated to use gl_FragCoord for reliable pixel lookup and simplified Bayer logic
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    // Dithering Logic
    if (lum < step1) {
        // Deep only (or fade to black if desired, but sticking to brand deep)
        finalColor = uColor1;
    } else if (lum < step2) {
        // Deep -> Primary
...
class DitherEffectImpl extends Effect {
  ...
  update(renderer: any, inputBuffer: any, deltaTime: number) {
     ...
     if (uniforms && uniforms.get('uTime')) {
         uniforms.get('uTime').value += deltaTime * 0.5;
     }
  }
}
...
export const DitherEffect = forwardRef(({ }, ref) => {
  ...
  useEffect(() => {
    ...
    const uniforms = (effect as any).uniforms;
    if(uniforms) {
