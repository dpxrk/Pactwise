...
CREATE OR REPLACE FUNCTION auth.has_role_any(required_roles user_role[])
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE POLICY "Users can create contracts for their enterprise" ON contracts
    ...
    WITH CHECK (
        enterprise_id = auth.user_enterprise_id()
        AND auth.has_role_any(ARRAY['user', 'manager', 'admin', 'owner']::user_role[])
    );
...
CREATE POLICY "Managers can update contracts" ON contracts
    ...
    USING (
        enterprise_id = auth.user_enterprise_id()
        AND auth.has_role_any(ARRAY['manager', 'admin', 'owner']::user_role[])
        AND deleted_at IS NULL
    )
    WITH CHECK (
        enterprise_id = auth.user_enterprise_id()
...
CREATE POLICY "Users can view their enterprise's departments" ON departments
    FOR SELECT
    USING (enterprise_id = auth.user_enterprise_id());
...
CREATE POLICY "Admins can manage departments" ON departments
    ...
    USING (
        enterprise_id = auth.user_enterprise_id()
        AND auth.has_role_any(ARRAY['admin', 'owner']::user_role[])
    );
    ...
    WITH policy_analysis AS (
        SELECT 
            ...
            CASE 
                ...
                WHEN p.polqual::text LIKE '%auth.user_enterprise_id()%' THEN 10
                ELSE 5
...
GRANT EXECUTE ON FUNCTION auth.user_enterprise_id() TO authenticated;
GRANT EXECUTE ON FUNCTION auth.user_role() TO authenticated;
GRANT EXECUTE ON FUNCTION auth.has_role(user_role) TO authenticated;
GRANT EXECUTE ON FUNCTION auth.has_role_any(user_role[]) TO authenticated;

-- Grant select on materialized view
GRANT SELECT ON user_permissions TO authenticated;

-- Comments for documentation
COMMENT ON FUNCTION auth.user_enterprise_id() IS 'Cached lookup of current user enterprise_id for RLS policies';
COMMENT ON FUNCTION auth.user_role() IS 'Cached lookup of current user role for RLS policies';
COMMENT ON FUNCTION auth.has_role(user_role) IS 'Check if current user has specific role';
COMMENT ON FUNCTION auth.has_role_any(user_role[]) IS 'Check if current user has any of the specified roles';
