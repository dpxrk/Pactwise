...
INCLUDE (name, risk_level, total_contract_value)
WHERE deleted_at IS NULL;

-- Budget monitoring with alerts
CREATE INDEX IF NOT EXISTS idx_budgets_alerts 
ON budgets(enterprise_id, status, alert_threshold) 
WHERE deleted_at IS NULL 
...
INCLUDE (agent_type, input_data, enterprise_id)
...
ON contracts(enterprise_id, end_date, auto_renew_notification_date) 
WHERE deleted_at IS NULL 
  AND status = 'active' 
...
ON vendors(enterprise_id, risk_level, last_review_date) 
WHERE deleted_at IS NULL 
  AND status = 'active'
  AND risk_level IN ('high', 'critical')
  AND (last_review_date IS NULL OR last_review_date < NOW() - INTERVAL '30 days');

-- Pending AI analysis queue
CREATE INDEX IF NOT EXISTS idx_contracts_pending_ai_analysis 
ON contracts(enterprise_id, created_at, analysis_priority) 
WHERE deleted_at IS NULL 
...
ON contracts USING gin(
  (
    ...
    setweight(to_tsvector('contract_search', coalesce(description, '')), 'B') ||
    setweight(to_tsvector('contract_search', coalesce(notes, '')), 'C')
  )
) WHERE deleted_at IS NULL;
...
SELECT 
    ...
    AVG(c.risk_score) FILTER (WHERE c.status = 'active') as avg_contract_risk,
    -- Vendor statistics
    ...
    COUNT(DISTINCT v.id) FILTER (WHERE v.risk_level IN ('high', 'critical')) as high_risk_vendors,
    -- Budget statistics
    COUNT(DISTINCT b.id) FILTER (WHERE b.status = 'active') as active_budgets,
    SUM(b.allocated_amount) FILTER (WHERE b.status = 'active') as total_budget_allocated,
    SUM(b.spent_amount) FILTER (WHERE b.status = 'active') as total_budget_spent,
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE status = 'signed') as contracts_signed,
